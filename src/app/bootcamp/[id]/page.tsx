...
export default function BootcampPage({ params }: BootcampPageProps) {
  const bootcamp = bootcamps.find((b) => b.id === params.id)
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2">
          <div className="h-64 overflow-hidden relative mb-8">
            <Image
              src={bootcamp.image || `/placeholder.svg?height=400&width=800&text=${bootcamp.location}`}
          ...
          <div className="mb-8">
            <h1 className="text-3xl font-bold mb-4">{bootcamp.location} Bootcamp</h1>
                ...
                {bootcamp.students.map((student) => (
                  <Card key={student.id}>
                    <div className="h-32 w-32 overflow-hidden rounded-full relative">
                      <Image
                        src={student.image || `/placeholder.svg?height=200&width=200&text=${student.name}`}
                        alt={student.name}
                        fill
                        className="object-cover"
                      />
              ...
              {bootcamp.sponsors.map((sponsor) => (
                <div key={sponsor.id} className="flex items-center space-x-4">
                  <div className="h-12 w-12 overflow-hidden rounded relative">
                    <Image
                      src={sponsor.logo || `/placeholder.svg?height=50&width=50&text=${sponsor.name}`}
                      alt={sponsor.name}
                      fill
